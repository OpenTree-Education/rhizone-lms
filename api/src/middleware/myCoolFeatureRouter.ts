...
import { getCurrentPage, setCurrentPage } from '../services/myCoolFeatureService';
...
myCoolFeatureRouter.patch(
  ...
  async (req, res, next) => {
    ...
    res.status(200).json(itemEnvelope({principal_id: principalId, page_number: currentPage}));
  }
);
