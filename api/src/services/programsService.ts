...
export const listCurriculumActivities = async (curriculumId?: number) => {
    ...
    if (curriculumId) {
        foundActivities = await db('activities').select('activities.title', 'activities.description_text', 'activities.curriculum_week', 'activities.curriculum_day', 'activities.start_time', 'activities.end_time', 'activities.duration', 'activity_types.title as activity_type').leftJoin('activity_types', 'activities.activity_type_id', 'activity_types.id').where({curriculum_id: curriculumId});
    } else {
